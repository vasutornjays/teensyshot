...
// Error handling
#define ESCCMD_ERROR(code)           \
    {                                \
        ESCCMD_last_error[i] = code; \
        return code;                 \
    }
...
int ESCCMD_extract_packet_data(uint8_t i)
{
    ...
    if (!ESCCMD_tlm_valid[i])
    {
        ...
        ESCCMD_last_error[i] = ESCCMD_ERROR_TLM_CRC; // return -5 

        // Check for excessive CRC errors
        if (ESCCMD_CRC_errors[i] >= ESCCMD_TLM_MAX_CRC_ERR) // Too much CRC Error
        {
    ...
    if (ESCCMD_tlm_emu_nb[i] < ESCCMD_EMU_TLM_MAX)
    {
        if (ESCCMD_tlm[i])
        {
            ...
            if (local_state & ESCCMD_STATE_3D)
            {
                ...
                {
                    // Negative direction
                    ESCCMD_tlm_emu_rpm[ESCCMD_tlm_emu_nb[i]][i] = (uint16_t)((float)(ESCCMD_cmd[i] - (DSHOT_CMD_MAX + 2 + ESCCMD_MAX_3D_THROTTLE)) / ESCCMD_MAX_3D_THROTTLE * (float)(ESCCMD_EMU_TLM_VOLT / 100) * ESCCMD_EMU_MOTOR_KV / 100.0 * ESCCMD_TLM_NB_POLES / 2);
                }
                else
                {
                    // Positive direction
                    ESCCMD_tlm_emu_rpm[ESCCMD_tlm_emu_nb[i]][i] = (uint16_t)((float)(ESCCMD_cmd[i] - (DSHOT_CMD_MAX + 1)) / ESCCMD_MAX_3D_THROTTLE * (float)(ESCCMD_EMU_TLM_VOLT / 100) * ESCCMD_EMU_MOTOR_KV / 100.0 * ESCCMD_TLM_NB_POLES / 2);
                }
            }
            else
            {
                // Normal mode
                ESCCMD_tlm_emu_rpm[ESCCMD_tlm_emu_nb[i]][i] = (uint16_t)((float)(ESCCMD_cmd[i] - (DSHOT_CMD_MAX + 1)) / ESCCMD_MAX_THROTTLE * (float)(ESCCMD_EMU_TLM_VOLT / 100) * ESCCMD_EMU_MOTOR_KV / 100.0 * ESCCMD_TLM_NB_POLES / 2);
            }
        }
...
int ESCCMD_tic(void)
{
    ...
    if (!ESCCMD_timer_flag)
    {
        return 0; // If timer not started return 0
    }
    ...
    for (i = 0; i < ESCCMD_n; i++)
    {
        ...
        while (ESCCMD_read_packet(i)) // If cannot read packet return NULL
        {

            // Update pending packet counter
            if (ESCCMD_tlm_pend[i])
            {
                ESCCMD_tlm_pend[i]--; // Decrease number of pending package if avaliable
            }
            else
            {
                // Spurious packet ?
                ESCCMD_last_error[i] = ESCCMD_ERROR_TLM_PEND; // return -7
            }

            // Extract packet data
            ESCCMD_extract_packet_data(i);
            // If telemetry is not valid (only CRC Check error)
                // return ESCCMD_ERROR_TLM_CRC (-5) if CRC check error  
                // return ESCCMD_ERROR_TLM_CRCMAX (-9) when get too much CRC check error
                // Flush UART incoming packet, reset ESCCMD_tlm_pend and ESCCMD_tlm_lost_cnt
            // Else if overheating > 100c
                // return ESCCMD_ERROR_TLM_TEMP (-8)
            // Else
                // return 0 if OK.
        }
        ...
        if (ESCCMD_tlm_pend[i] > ESCCMD_TLM_MAX_PEND)
        {
            ...
            {
                ESCCMD_last_error[i] = ESCCMD_ERROR_TLM_LOST; // return -10
            }
            else
            {
            ...
            // Decrement packet pending counter (packet is loss)
            ESCCMD_tlm_pend[i]--;
        }
    }
    ...
    if (local_tic_pend)
    {
        ...
        if (!(ESCCMD_tic_counter % ESCCMD_TLM_PER)) // When clear all error status
        {
            ...
            ESCCMD_tic_counter = 0; // Reset ESCCMD_tic_counter to prevent overflow
        }
        ...
        if (!ESCCMD_init_flag)
        {
            for (i = 0; i < ESCCMD_n; i++)
            {
                ESCCMD_last_error[i] = ESCCMD_ERROR_INIT; // If ESC is not init return -3
            }
            return ESCCMD_TIC_OCCURED; // Return 1 as tic is occurred but ESCCMD_last_error = -3
        }
        ...
        for (i = 0; i < ESCCMD_n; i++)
        {
            if (!(ESCCMD_state[i] & ESCCMD_STATE_ARMED))
            {
                ESCCMD_last_error[i] = ESCCMD_ERROR_SEQ; // If ESC is not init return -2
                return ESCCMD_TIC_OCCURED; // Return 1 as tic is occurred but ESCCMD_last_error = -2
            }
        }
        ...
        for (i = 0; i < ESCCMD_n; i++)
        {
            if (ESCCMD_throttle_wd[i] >= ESCCMD_THWD_LEVEL) // If number of watchdog counter exceed the level
            {
                // Watchdog triggered on ESC number i 
                ESCCMD_cmd[i] = DSHOT_CMD_MOTOR_STOP; // Stop motor
                ESCCMD_tlm[i] = 0;
                ESCCMD_last_error[i] = 0;
                ESCCMD_tlm_lost_cnt[i] = 0;
                ESCCMD_CRC_errors[i] = 0;
                noInterrupts();
                ESCCMD_state[i] &= ~(ESCCMD_STATE_START); // Set ESC state to STOP
                interrupts();
            }
            else
            {
                ESCCMD_throttle_wd[i]++; // increase wd count
            }
        }

        bool is_tlm_req = !(ESCCMD_tic_counter % ESCCMD_TLM_REQ_FREQ);
